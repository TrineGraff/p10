...
v_l = 1/abs(coef_lasso[idx_lasso]) 
...
idx_adap_ols = which(coef_adap_ols != 0) 
coef_adap_lasso = c(rep(0, 21), coef_adap_lasso_vec[3], coef_adap_lasso_vec[4], rep(0, 103))
...
idx = which(test != 0)
test = coef(fit_lasso , s = fit_bic_lasso$lambda)

...
coef_ridge = as.vector(coef(ridge_fit, s = ridge_cv$lambda.1se)) %>% .[-1]
idx_ridge = which(coef_ridge != 0) 
coef_ridge[idx_ridge]  


# plot --------------------------------------------------------------------
coef = data.table("Ridge" = coef_ridge)
...
g1 = grid.draw(cbind(p1, p2, p3, p4, size = "first"))
grid.draw(cbind(p5, p6, p7, p8, size = "first"))              

...
data.frame(
  ...
  error = c(lars_cv$cv[getmin$idx_min], lars_cv$cv[getmin$idx_1se]),
  p = c(parm(coef(lars_fit, s = getmin$lambda.min, mode = "step")), 
        parm(coef(lars_fit, s = getmin$lambda.1se, mode = "step")) ) 
  #fraction fordi vi har L1 norm af vektoren (se plot)
)

beta_hat = as.vector(coef(lasso_fit, s = getmin$lambda.min, mode = "fraction"))

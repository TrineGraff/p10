...
\end{align*}
Derfor kan faktor loadings \(\tLambda\) kun bestemmes op til en ortogonal matrix \(\tR\).
...
Lad \(\widehat{\tbeta} = \del{\widehat{\tbeta}_{\tF}^T \widehat{\tbeta}_{\tw}^T}^T\) betegne OLS estimaterne af \(\tbeta = \del{\tbeta_{\tF}^T \tbeta_{\tw}^T}^T\) fra regressionen af \(y_{t+h}\) på \(\widehat{\tz}_t^T\) for \(t=1, \ldots, T-h\).

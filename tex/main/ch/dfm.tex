...
\end{defn}
%
I kapitlet betragter vi også faktor modellen på formen
\begin{align*}
x_{jt} = \boldsymbol{\lambda}_j \tF_t + e_{jt} = \lambda_{jm}F_{jt} + e_{jt}, \quad j = 1, \ldots, p,
\end{align*} 
...
Vi betragter også faktor modellen på matrix form
\begin{align*}
...
I dette underafsnit introduceres nogle antagelser, som sikrer at estimatoren er entydig.
Der gælder, at faktor loadings ikke er entydige, som vi først vil beskrive. 
...
og finder, at
\begin{align*}
\boldsymbol{\Sigma}_{\X \X} &= \text{cov} \sbr{\X_t} = \E{\X_t \X_t^T} \\
...
\end{pmatrix}
+ \begin{pmatrix}
T^{-1} \sum_{t=1}^T \mathbf{S} \widehat{\tF}_t \epsilon_{t+h} \\ T^{-1} \sum_{t=1}^T \tw_t \epsilon_{t+h}
\end{pmatrix}\\
&= \begin{pmatrix}
T^{-1} \sum_{t=1}^T \widehat{\tF}_t \widehat{\tF}_t^T & T^{-1} \mathbf{S} \sum_{t=1}^T \widehat{\tF}_t \tw_t^T \\
T^{-1} \sum_{t=1}^T \tw_t \widehat{\tF}_t^T \mathbf{S} & T^{-1} \sum_{t=1}^T \tw_t \tw_t^T

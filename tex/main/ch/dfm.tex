...
\textit{I dette kapitel gives en kort introduktion af faktor modellen. Kapitlet er hovedsagligt baseret på \citep{stock_watson_2002a} og kapitel 9 i \citep{faktorbook}.} \\[4mm]
%
...
Derudover betragtes faktor modellen på matrix-vektor form
\begin{align*}
\X = \tF \tLambda^T + \mathbf{e},
\end{align*}
hvor \(\X\) og \(\mathbf{e}\) er \(T \times p\) matricer med \(t\)'te række henholdsvis \(\X_t^T\) og \(\te_t^T\) og \(\tF\) er en \(T \times r\) matrix med \(t\)'te række \(\tF_t^T\).
...
Proceduren transformerer et antal korreleret variable til et mindre antal ukorreleret variable, som kaldes principale komponenter.
...
\end{align*}
Lad \(p^{-1} \tLambda^T \tLambda = \mathbf{I}_r\), som antages i antagelse \ref{ass:faktor}a), da har vi, at \(\widehat{\tF} = p^{-1} \X \tLambda\).
...
%
For prædiktionsligningen \eqref{eq:factor_model_forecast} antages, at hvis \(\tF_t\) er observeret, da vil OLS give konsistente estimatorer af regressionskoefficienterne.
...
\end{ass}
%
Antagelse \ref{ass:forecasting_ligning}.a)-\ref{ass:forecasting_ligning}.c) medfører, at regressionen af \(y_{t+h}\) på \(\tz_t^T\) giver konsistente OLS estimatorer givet ved \(\widehat{\tbeta} = \del{\sum_{t=1}^{T-h} \widehat{\tz}_t \widehat{\tz}_t^T}^{-1} \del{\sum_{t=1}^{T-h} \widehat{\tz}_t y_{t+h}}\).
...
Lad \(\widehat{\tbeta}_{\tF}\) og \(\widehat{\tbeta}_{\tw}\) betegne OLS estimaterne af \(\tbeta_{\tF}\) og \(\tbeta_{\tw}\) fra regressionen af \(y_{t+h}\) på \(\widehat{\tz}_t^T\) for \(t=1, \ldots, T-h\). Da gælder følgende
\begin{enumerate}[label=\alph*)]

...
\begin{align*}
\argmin_{\tbeta \in \R^p} \cbr{\Vert \y - \X \tbeta \Vert_2^2 + \lambda \sum_{j=1}^p w_j \abs{ \beta_j}},
\end{align*}
...
Vælg \(\gamma>0\) og definer \(\widehat{\mathbf{w}} = \frac{1}{\abs{ \tilde{\tbeta} }^\gamma}\), da er adaptive lasso estimaterne givet ved
\begin{align}
\widehat{\tbeta}^\text{AL} = \argmin_{\tbeta \in \R^p} \cbr{ \Vert \y - \X \tbeta \Vert_2^2 + \lambda \sum_{j=1}^p \frac{ \abs{ \beta_j }}{\abs{ \tilde{\beta}_j }^\gamma}}. \label{eq:4.76}
\end{align}
\end{defn}

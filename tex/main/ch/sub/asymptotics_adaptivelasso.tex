...
\begin{align*}
\widehat{\tbeta}^\text{AL} = \argmin_{\tbeta \in \R^p} \cbr{ \Vert \y - \sum_{j=1}^p \x_j \beta_j \Vert_2^2 + \lambda_n \sum_{j=1}^p \widehat{w}_j \abs{ \beta_j}},
\end{align*}
hvor \(\widehat{w}_j = \frac{1}{\abs{\tilde{\beta}_j}^\gamma}\) for \(\gamma > 0\) og \(\lambda_n\) varierer med \(n\).

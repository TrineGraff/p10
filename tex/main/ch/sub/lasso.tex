...
Værdien af \(\lambda\) skal specificeres ved en ekstern procedure kaldet \textit{krydsvalidering}, som vil blive diskuteret i kapitel \ref{kap:statistisk_inferens}.
...
\begin{align} 
\widehat{\tbeta}^\text{ridge} = \argmin_{\tbeta} \cbr{\sum_{i=1}^n \del{y_i - \sum_{j=1}^p x_{ij} \beta_j}^2}, \ \text{underlagt at } \sum_{j=1}^p \beta_j^2 \leq t, \label{eq:2.7} 
\end{align} 
...
\begin{align*}
\widehat{\tbeta}^\text{ridge} = \argmin_{\tbeta} \cbr{ \Vert \y - \X \tbeta \Vert_2^2 + \lambda \Vert \tbeta \Vert_2^2},
\end{align*}
hvor $\lambda \geq 0$.
...
%
\begin{exmp}
Lad os betragte data givet i tabel \ref{tab:crime} af \citep{crimedata}.
...
Heraf ses at lasso udfører variabeludvælgelse og shrinks koefficienterne, mens ridge regression blot shrinks koefficienterne.
%
\imgfigh{crime_lasso_ridge.pdf}{0.9}{Koefficientstierne for lasso og ridge regression plottet imod $\log \del{\lambda}$.}{crime_koef}
...
Strafleddet for lasso problemet er ikke differentiabel, og derfor findes der ikke en lukket løsning til optimeringsproblemet.
...
%Gentagne cycle gennem prædiktorerne i en fast, men vilkårlig orden $j=1, 2, \ldots, p$, hvor koefficienten $\beta_j$ opdateres i det $j$'te step, ved at minimere objektfunktionen i dens koordinat, mens de resterende koefficienter $\cbr{\widehat{\beta}_k, k \neq j}$ fastholdes deres nuværende værdier. 
...
\end{align*}
Definer den partialle residual $r_i^{(j)}=y_i - \sum_{k \neq j} x_{ik} \widehat{\beta}_k$, som fjerner nuværende fit fra den $j$'te prædiktor.
...
\end{align}
dermed er $\widehat{\beta}_j$ blot soft-thresholded version af det univariate mindste kvadraters estimat af $\mathbf{y}$ regresseret imod $\mathbf{x}_j$, som bevises i ---. Således har vi en lukket løsning og ingen iterationer er påkrævet.
...
Lad \(\widehat{\y}\) betegne \(n\) sample prædiktorer, da defineres 
\begin{align}

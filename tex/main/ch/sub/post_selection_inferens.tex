\section{Generelt scheme for post-selection inferens} \label{subsec:teste_polyhedron}
I dette afsnit vil vi diskutere et generelt scheme for inferens efter variabeludvægelse, som giver eksakte \(p\)-værdier og konfidensintervaller i det gaussiske tilfælde.
Afsnittet er baseret på \citep{post_inference} og kapitel 6 i \citep{hastie}.

Dette scheme kan håndtere enhver procedure, hvor udvælgelsen af variable kan karakteriseres ved en mængde af lineære uligheder i \(\y\).
Mere specifik skal udvælgelsen af variable kunne skrives som \(\cbr{\boldsymbol{\Gamma} \y \geq \mathbf{u}}\), for en matrix \(\boldsymbol{\Gamma}\) og vektor \(\mathbf{u}\).
...
\end{align}
Da er \(I= [\delta_\alpha, \infty )\) et gyldig one-sided konfidensinterval for \(\teta^T \tmu\) betinget \(\boldsymbol{\Gamma} \y \geq \mathbf{u}\)
\begin{align}
\mathbb{P} \del{\boldsymbol{\eta}^T \tmu \geq \delta_\alpha \given \boldsymbol{\Gamma} \y \geq u} = 1- \alpha. \label{eq:post_1.17}
\end{align}
\end{lem}
%
Af teststørrelsen i \eqref{eq:post_1.14} har vi magt imod one-sided alternativ hypotese \(\hyp_1 : \teta^T \tmu > 0\), da den trunkerede normal overlevelsesfunktion \(1-F_{\mu, \sigma^2}^{\sbr{a,b}} \del{x}\), evalueret i ethvert fast punkt \(x\), er monoton stigende i \(\mu\).
Dette gælder også for konfidensintervallet i \eqref{eq:post_1.16} og \eqref{eq:post_1.17}. TJA?
...
For \(\teta = \del{\X_{\A_k}^+}^T \mathbf{e}_k\) fås, at
\begin{align*}
\teta^T \tmu = \mathbf{e}_k^T \X_{\A_k}^+ \tmu =  \mathbf{e}_k^T \del{\X^T \X}^{-1} \X^T \X \tbeta = \beta_j ,
\end{align*}
således at nulhypotesen \(\hyp_0: \mathbf{e}_k^T \X_{\A_k}^+ \tmu= 0\), svarer til at teste om koefficienten for den sidst tilføjede variabel er lig 0.
Men for dette valg af \(\teta\) er det one sided setup givet ved \(\hyp_0: \teta^T \tmu = 0\) og  \(\hyp_0: \teta^T \tmu > 0\), som ikke giver mening, da der ikke er grund til at tro at \(k\)'te regressionskoefficient \(\mathbf{e}_k^T \X_{\A_k}^+ \boldsymbol{\mu}\) skal være positiv.
Definer istedet \(\teta = s_k \del{\X_{\A_k}^+}^T \mathbf{e}_k\), hvor \(s_k\) er fortegnet af \(k\)'te variabel når den medtages i LARS eller lasso modellen, da er nulhypotesen \(\hyp_0: s_k \mathbf{e}_k^T \X_{\A_k}^+ \tmu = 0\) uændret, men den alternative hypotese \(\hyp_0: s_k \mathbf{e}_k^T \X_{\A_k}^+ \tmu > 0\) har nu en konkret fortolkning.
